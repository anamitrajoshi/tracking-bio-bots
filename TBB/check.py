...
def play():
    ...
    def count_stuck_squares(rows, columns, wall_vals):
        # ... (Same count_stuck_squares function as before) ...
        blocked_cells = []
    ...
    def bot_movement(grid, keys_pressed, bot, wall_vals):
        ...
        if keys_pressed[pygame.K_UP] and bot.y - 1 > 0:
            new_bot.y -= cell_size
        if bot.x == WIDTH - BOT_WIDTH and bot.y == 0:
            pygame.quit()  # Exit the game when the bot reaches the top-left corner
        ...
        for wall in wall_vals:
            ...
            if new_bot.colliderect(wall_rect):
                stuck_timer = FPS * 5  # Set the stuck timer to 5 seconds (assuming FPS is 10)
                return
    ...
    def main():
        global screen, cell_size, rows, columns, wall_vals
        ...
        while running:
            ...
            highlight_stuck_squares(stuck_squares_count)
